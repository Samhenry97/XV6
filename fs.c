...
iinit(int dev)
{
  ...
  readsb(dev, &sb);
  cprintf(ALL_DEVS, "sb: size %d nblocks %d ninodes %d nlog %d logstart %d\
...
itrunc(struct inode *ip)
{
  ...
  if(ip->addrs[NDIRECT]){
    ...
    if(a[NINDIRECT-1] != 0){
      uint a_org = a[NINDIRECT-1];
      bp = bread(ip->dev, a[NINDIRECT-1]);
      a = (uint*)bp->data;
